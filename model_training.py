...
import json, logging, argparse, time, torch, joblib, os
...
def data_pipeline(logger: Logger, dataset_url: str, root_data_dir: str= "../Data", data_file_path: str="Dataset.csv", data_splits_dir: str="DataSplits", scaler_dir = "Scalers", target_column: str="Target", extra_dropped_columns: Optional[List[str]]=None, batch_size: int=64, num_workers: int=0, pin_memory: bool=False, drop_last: bool=True) -> tuple[Dataset, Dataset, Dataset, DataLoader, DataLoader, DataLoader, MinMaxScaler, MinMaxScaler]:
    ...
    if os.path.exists(TRAIN_DATA_PATH) and os.path.exists(TEST_DATA_PATH) and os.path.exists(VALIDATION_DATA_PATH) :
        ...
        feature_scaler = MinMaxScaler() # Create the scaler objects
        ...
        logger.info(f"Train Labels Scaled Shape: {Y_test_scaled.shape}")
        logger.info(f"Validation Features Scaled Shape: {X_validation_scaled.shape}")
        logger.info(f"Validation Labels Shape: {Y_validation_scaled.shape}")
        logger.info(f"Test Features Scaled Shape: {X_test_scaled.shape}")
        logger.info(f"Test Labels Scaled Shape: {Y_test_scaled.shape}")
    ...
    train_dataset = OilDataset(csv_file=TRAIN_DATA_PATH, label_column=target_column)
    test_dataset = OilDataset(csv_file=TEST_DATA_PATH, label_column=target_column)
    val_dataset = OilDataset(csv_file=VALIDATION_DATA_PATH, label_column=target_column)
...
class Agent(torch.nn.Module):
    ...
    def forward(self, x):
        """Forward pass of Agent's Layers."""
        x = self.linear(x)
        x = self.layers(x)
        x = self.out(x)
        return x
...
def train_model(logger: Logger, model_config: dict, train_dataloader: DataLoader, validation_dataloader: DataLoader, model: Agent = None, epochs=32, learning_rate=0.0003, max_grad_norm=0.5, log_iterations=10, eval_iterations=10, device="cpu") -> tuple[Agent, dict]:
    ...
    for epoch in tqdm(range(epochs), desc=f">>>>>>>>>>>>>>>>>>>>>\nMain Epoch (Outer Loop)", leave=True):
        ...
        for batch_idx, (inputs, labels) in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch + 1}/{epochs} - Training", leave=False)):           # Get a mini-batch of training examples from the dataloader
            ...
            inputs, labels = inputs.to(device), labels.unsqueeze(dim=-1).to(device)   # Move the inputs and labels to the device (turning labels into a column vactor)
            ...
            if (batch_idx + 1) % log_iterations == 0:
                log_iteration(logger=logger, batch_idx=(batch_idx + 1), total_batches=train_dataloader_length, loss_value=loss.item())
        ...
        if (epoch + 1) % eval_iterations == 0:
            val_loss = evaluate_model(logger=logger, model=agent, dataloader=validation_dataloader, current_epoch=(epoch + 1), max_epochs=epochs, device=device)
...
def main(parser_args, global_config) -> int:
    ...
    logger = logging.getLogger(global_config["logging"]["logger_name"])
    logger.info("STARTING MAIN FUNCTION")
    
    if parser_args.device:     # Check if the user specified to use a CPU or GPU for training
    ...
    try:
        ...
        (train_dataset, test_dataset, validation_dataset, train_dataloader, test_dataloader, validation_dataloader, feature_scaler, label_scaler) = data_pipeline(logger=logger, **PIPELINE_CONFIG, batch_size=parser_args.dataloader_batch_size, num_workers=parser_args.dataloader_num_workers, pin_memory=parser_args.dataloader_pin_memory, drop_last=True)
...
if __name__ == '__main__':
    ...
    logger_name = json_args['logging']['logger_name']   # Get the logger name
    log_to_file = json_args['logging']['log_to_file']   # Set whether to log to a logfile or not
    log_file = json_args['logging']['log_file'] # Get the log file path

    log_lvl = json_args['logging']['log_level']
    log_level = getattr(logging, log_lvl.upper(), logging.INFO) # Set fallback if invalid input
    log_mode = json_args['logging']['log_mode'] # Set the log file mode
    ...
    # Clear existing logging handlers. USE if re-running in an interactive environment and changing logging settings.
    # for handler in logging.root.handlers[:]:
    #     logging.root.removeHandler(handler)

    # ---  Log Handlers Check and Creation --- 
    if not log_to_console and not log_to_file:
        print("Warning: No logging handlers configured. Verbose Logging will be disabled. Training Progress will still be shown using tqdm. In 'config/config.json', set ['log_to_file': true] or ['log_to_console': true] if you want to change the logging behavior.", flush=True)
    else:
    ...
    logging.basicConfig(level=log_level, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S',handlers=handlers)

    logger = logging.getLogger(name=logger_name)    # Initialize a singleton logger instance using the specified logger name
    ...
    # Call the main function with the both the parser arguments and the 'config.json' file arguments
    ...
    if ret == 0:
        if not log_to_console and not log_to_file:
            print("FINISHED MAIN SCRIPT.\nCheck '/Models' folder for saved model(s)", flush=True)
        else:
